...
def determine_affordance(image_path, part, instruction, number_list):
    ...
    response = create_chat_completion(
        ...
        messages=[
            ...
            {"role": "user", "content": [
                {"type": "text", "text": f"""This image was taken by the camera on you. 
                                             The object in the red bounding box is your target object for operation.
                                             Based on this image, please select a point that is closest to the {part} of the target object and is suitable for manipulation.
                                             Compare all candidate points and select the single most suitable point that is closest to the {part} of the target object 
                                             In the image, candidate points on the target object have been labeled with IDs {number_list}. 
                                             Please select a single candidate point 
                                             that meets the requirements above and provide the selected point's ID. 
                                             Please make sure the point's ID on the image and must output the result as follows: {{"id":id}}."""},
                {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64_image}"}}
            ]}
        ]
