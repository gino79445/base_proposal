...
def sample_gaussian_actions(
    ...
    while i < num_samples:
        ...
        x_map = int(point_3d[0] / 0.05 + 101)
        y_map = int(point_3d[1] / 0.05 + 101)
        time += 1
        if time > 10000:
            break
...
def update_gaussian_distribution(best_actions_positions, std_dev, std_dev_decay=0.6):
    """
...
def get_base(image, instruction, depth_image, obstacle_map, R, T, fx, fy, cx, cy, K=3):
    # 讀取圖片
    ...
    for p in range(parrallel):
        ...
        for i in range(iterations):
            ...
            # write the image
            cv2.imwrite(f"./annotation/annotated_image_{i + 1}.png", annotated_image)
            result = get_point(
                f"./annotation/annotated_image_{i + 1}.png", instruction, K
            )
            best_indices = result
            # print(actions)
            # print(best_indices)
    ...
    base_pixel = (position[0], position[1])

    base_point = get_3d_point(
        w - base_pixel[0],
        h - base_pixel[1],
        depth_image[base_pixel[1], base_pixel[0]],
        R,
        T,
        fx,
        fy,
