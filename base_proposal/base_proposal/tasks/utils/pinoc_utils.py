...
def get_se3_err(pos_first, quat_first, pos_second, quat_second):
    ...
    rot_first = Rotation.from_quat(
        np.array([quat_first[1], quat_first[2], quat_first[3], quat_first[0]])
    ).as_matrix()  # Quaternion in scalar last format!!!
    rot_second = Rotation.from_quat(
        np.array([quat_second[1], quat_second[2], quat_second[3], quat_second[0]])
    ).as_matrix()  # Quaternion in scalar last format!!!
...
class PinTiagoIKSolver(object):
    def __init__(
        ...
        # Settings
        self.damp = (
            1e-10  # Damping co-efficient for linalg solve (to avoid singularities)
        )
        ...
        if self._include_base:
            ...
            jointsOfInterest = (
                ...
                + jointsOfInterest
            )  # 10 DOF with holo base joints included (11 with torso)
        ...
        for jnt in jointsOfInterest:
            if self.model.existJointName(jnt):
                ...
                print(
                    "[IK WARNING]: joint " + str(jnt) + " does not belong to the model!"
                )
        ...
        self.model = pin.buildReducedModel(
            self.model,
            jointIdsToExclude[1:].tolist(),
            reference_configuration=reference_configuration,
        )
        # for joint in self.model.joints:
        #    print(joint)
        assert len(self.model.joints) == (len(jointsOfInterest) + 1), (
            "[IK Error]: Joints != nDoFs"
        )
        ...
        if self._include_base:
            # self.joint_pos_min = np.hstack((-0.0, -0.0,1 , -0.577, self.joint_pos_min))
            # self.joint_pos_max = np.hstack((0.0, 0.0, 1,0.577 , self.joint_pos_max))
            # 30
            # self.joint_pos_min = np.hstack((-0.0, -0.0, 0.866 ,-0.5, self.joint_pos_min))
            # self.joint_pos_max = np.hstack((0.0, 0.0, 1, 0.5, self.joint_pos_max))
            # 15
            self.joint_pos_min = np.hstack((-3, -3, 0, -1, self.joint_pos_min))
            self.joint_pos_max = np.hstack((3, 3, 1, 1, self.joint_pos_max))
            # 20
            # self.joint_pos_min = np.hstack((-0.0, -0.0, 0.94 ,-0.342, self.joint_pos_min))
            # self.joint_pos_max = np.hstack((0.0, 0.0, 1, 0.342, self.joint_pos_max))
        self.joint_pos_mid = (self.joint_pos_max + self.joint_pos_min) / 2.0
        # Get End Effector Frame ID
        self.id_EE = self.model.getFrameId(name_end_effector)

    def set_Rmin_Rmax(self, Rmin, Rmax):
        self.joint_pos_min[0:4] = Rmin
        self.joint_pos_max[0:4] = Rmax
    ...
    def solve_ik_pos_tiago(
        ...
        dt=0.1,
        pos_threshold=0.05,
        angle_threshold=15.0 * np.pi / 180,
        verbose=False,
        Rmin=[-4, -4, 0, -1],
        Rmax=[4, 4, 1, 1],
        quick=False,
        ...
        self.set_Rmin_Rmax(Rmin, Rmax)
        damp = 1e-10
        q_list = []
        success_list = []
        if des_quat is not None:
            # quaternion to rot matrix
            des_rot = Rotation.from_quat(
                np.array([des_quat[1], des_quat[2], des_quat[3], des_quat[0]])
            ).as_matrix()  # Quaternion in scalar last format!!!
        ...
        for n in range(n_trials):
            ...
            for i in range(800):
                ...
                if des_rot is None:
                    oMdes = pin.SE3(
                        oMf.rotation, des_pos
                    )  # Set rotation equal to current rotation to exclude this error
        ...
        if verbose:
            if success:
                ...
                print(
                    "[Warning: the IK iterative algorithm has not reached convergence to the desired precision]"
                )

        return success_list, q_list
