...
#                                            -0.8, 1.5708, 1.5708, 0], device=self._device)
        ...
        self.arm_left_start = torch.tensor([0.25, 1.5707, 1.5707,
                                            0.55, 1, -1.5707, 1.0], device=self._device)
        self.arm_right_start = torch.tensor([0.25, 1.5707, 1.5707,
                                            0.55, 1, -1.5707, 1.0], device=self._device)
...
#        self.robots.set_joint_positions(positions=jt_pos, joint_indices=self.base_dof_idxs)
    ...
    def apply_base_actions(self, actions):
        ...
        rotated_x = base_actions[0] * math.cos(jt_pos[2]) - base_actions[1] * math.sin(jt_pos[2])
        rotated_y = base_actions[0] * math.sin(jt_pos[2]) + base_actions[1] * math.cos(jt_pos[2])
    ...
    def get_upper_body_positions(self):
        ...
        combined_pos = self.robots.get_joint_positions(joint_indices=self.upper_body_dof_idxs, clone=True)
        return combined_pos
    ...
    def get_base_positions(self):
        ...
        combined_pos = self.robots.get_joint_positions(joint_indices=self.base_dof_idxs, clone=True)
        return combined_pos
    ...
    def set_arm_dof_init_pos(self):
        ...
        self.robots.set_joint_positions(positions=arm_left_pos, joint_indices=self.arm_left_dof_idxs)
        self.robots.set_joint_positions(positions=arm_right_pos, joint_indices=self.arm_right_dof_idxs)
